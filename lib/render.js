...
const checker = async ({type, renderTransparent, renderSides, noShadow, scale, reducer}) => {
  ...
  for (let i = 0; i < blocks.length; i += 1) {
    ...
    if (currImage.endsWith('bookshelf.png')){

      // oak_planks.png

      let upImage = currImage.replace('bookshelf', 'oak_planks');

      const side = fs.existsSync(upImage) ? upImage : currImage.replace('_top', '');
      await core(i === blocks.length - 1, reducer, type, scale, side, currImage);
