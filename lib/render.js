...
const checker = async ({type, renderTransparent, renderSides, noShadow, scale, reducer}) => {
    ...
    for (let i = 0; i < blocks.length; i += 1) {
        ...
        let realName = currImage.replace('grab/blocks/', '').replace('.png', '');

        if (realName in specialRenderItems) {
            let currentSpecialRender = specialRenderItems[realName];
            specialRender(currImage, realName, currentSpecialRender, type, scale, reducer, i, blocks);
            continue;
        }
...
const specialRender = (currentImage, currentName, currentSpecialRender, type, scale, reducer, i, blocks) => {
    let sideImage = currentImage.replace(currentName, currentSpecialRender.name);

    if (!fs.existsSync(sideImage)) {
        throw new Error(`Impossible to find the block ${sideImage}`);
    }

    let imgTop = (currentSpecialRender.type === 'side' ? sideImage : currentImage);
    let imgSide = (currentSpecialRender.type === 'side' ? currentImage : sideImage);

    console.log(imgTop + " - " + imgSide + " - " + (currentSpecialRender.type === 'side'));

    core(i === blocks.length - 1, reducer, type, scale, imgSide, imgTop);
}
