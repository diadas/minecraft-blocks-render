...
const checker = async ({type, renderTransparent, renderSides, noShadow, scale, reducer}) => {
    ...
    for (let i = 0; i < blocks.length; i += 1) {
        ...
        if (realName in specialRenderItems) {
            let upName = specialRenderItems[realName];
            specialRender(currImage, realName, upName, type, scale, reducer, i, blocks);
            continue;
        }
...
const specialRender = (currentImage, currentName, sideName, type, scale, reducer, i, blocks) => {
    let sideImage = currentImage.replace(currentName, sideName);

    if (!fs.existsSync(sideImage)) {
        throw new Error(`Impossible to find the block ${sideImage}`);
    }

    core(i === blocks.length - 1, reducer, type, scale, sideImage, currentImage);
}
